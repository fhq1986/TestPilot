...
| ![AI生成测试用例](docs/assets/screenshots/ai_testcase.png) | ![测试用例](docs/assets/screenshots/testcases.png) |
| **AI 生成测试用例** · 自然语言生成 / 需求文档导入 / Swagger 导入 | **测试用例** · Excel 模板导入 / 脚本导入 / 优先级 / AI 生成标记 / 不稳定用例 |
| ![执行记录](docs/assets/screenshots/executions.png) | ![执行详情](docs/assets/screenshots/execution-detail.png) |
| **执行记录** · 状态 / 触发方式 / AI 诊断入口 | **执行详情** · 步骤流 / trace 回放 / AI 根因分析 |
| ![测试计划](docs/assets/screenshots/test-plans.png) | ![计划详情](docs/assets/screenshots/plan-detail.png) |
| **测试计划** · 版本标识 / 目标通过率 / 多轮执行 | **计划详情** · 达标判定 / 轮次 / 验收报告 |
| ![数据集](docs/assets/screenshots/datasets.png) | ![视觉基线](docs/assets/screenshots/visual-baselines.png) |
| **数据集** · 数据驱动参数化（`{{变量}}`） | **视觉基线** · 按「用例 + 步骤」保存基线截图 |
| ![脚本录制](docs/assets/screenshots/recorder.png) | ![缺陷管理](docs/assets/screenshots/defects.png) |
| **脚本录制** · 操作实时转 Playwright 步骤 | **缺陷管理** · 失败步骤一键转缺陷 / 外部系统推送 |
| ![需求覆盖](docs/assets/screenshots/requirements.png) | ![测试套件](docs/assets/screenshots/suites.png) |
| **需求覆盖** · 覆盖率与未覆盖需求追踪 | **测试套件** · 用例归集 / 失败策略 / 一键运行 |
| ![定时任务](docs/assets/screenshots/schedules.png) | ![执行节点](docs/assets/screenshots/nodes.png) |
| **定时任务** · Cron 调度 + 执行范围编排 | **执行节点** · 分布式 Worker 在线状态与并发负载 |
