@echo off
rem ============================================================================
rem  TestPilot - Windows launcher (double-click entry from deploy\)
rem  Real logic lives in scripts/start-all.sh; this file only locates Git Bash
rem  and hands over. Keep this file ASCII-only: cmd.exe parses rem lines with
rem  the ANSI codepage BEFORE chcp takes effect, non-ASCII bytes get mangled
rem  and pieces of rem lines leak out as commands ('... is not recognized').
rem
rem  Usage: double-click deploy\start.cmd, or run from any dir:
rem           deploy\start.cmd [--no-build|--rebuild|--skip-db]
...
if not defined BASH_EXE (
  ...
  echo         or run manually in Git Bash:  bash scripts/start-all.sh
  echo.
  pause
  exit /b 1
)
...
cd /d "%AITEST_ROOT%"
"%BASH_EXE%" -c "exec bash scripts/start-all.sh %*"
